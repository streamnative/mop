...
public enum Mqtt5ConnReasonCode implements MqttReasonCode {
    ...
    public MqttConnectReturnCode convertToNettyKlass() {
        return Arrays.stream(MqttConnectReturnCode.values())
                .filter(returnCode -> this.byteValue() == returnCode.byteValue())
                .findFirst()
                .orElseThrow(() ->
                        new IllegalArgumentException(
                                String.format("illegal connection reason code here %s", this.code)));
    }
}
